...
impl From<MapHandlerError> for SdkError {
    fn from(err: MapHandlerError) -> Self {
        SdkError::JobExecutionFailed(anyhow::anyhow!(err.to_string()))
    }
}

/// Job ID for the map operation.
pub const JOB_MAP_ID: u64 = 4;

/// Handles the map job.
/// Takes a JSON string input via TangleArg, sends it to the Firecrawl map endpoint,
...
pub async fn handle_map(
    ...
    let input_json: Value =
        serde_json::from_str(&input).map_err(|_| MapHandlerError::InvalidInputJson)?;

    let client = reqwest::Client::new();
    let url = format!(
        "http://localhost:{}/api/v1/map", // Target map endpoint
    ...
    let response_bytes = serde_json::to_vec(&response_json).map_err(MapHandlerError::JsonParse)?;
    ...
    let ipfs_result = ctx
        ...
        .add(response_cursor)
        .await
        .map_err(|e| MapHandlerError::Ipfs(e.to_string()))?;
